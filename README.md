...
- **自动自愈**：被 FIFO 淘汰挤出白名单的设备，由其自身切网事件自动补回；蜂窝写入的 IP 在面板上有 📶 标记。
- **即时响应**：network-changed 事件即时触发，内置 1.5 秒网络就绪延时与自动重试机制。
- **智能检测**：每 5 分钟自动测试代理节点（默认 po0-hk-snell），仅在节点测试断连时才触发上报加白。
